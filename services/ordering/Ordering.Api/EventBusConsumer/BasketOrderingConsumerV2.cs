...
public class BasketOrderingConsumerV2 : IConsumer<BasketCheckoutEventV2>
{
    ...
    {
        _logger.LogInformation("BasketCheckoutEventV2 consumed for user {UserName}", context.Message.UserName);
        var command = _mapper.Map<CheckoutOrderCommandV2>(context.Message);
        var result = await _mediator.Send(command);
        _logger.LogInformation("BasketCheckoutEventV2 processed for user {UserName}", context.Message.UserName);
    }
}